...
*   **JSON Web Tokens (JWT)**: Secure, signed HS256 JWT access tokens (15-min expiry) and refresh tokens (7-day expiry) with token revoking/blacklisting on logout using Redis TTLs.
*   **Password Cryptography**: Salted bcrypt hashing via `passlib[bcrypt]`.
